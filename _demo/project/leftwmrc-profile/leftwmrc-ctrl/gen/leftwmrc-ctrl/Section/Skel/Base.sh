...
is_debug () {
	if [ "$DEBUG_LEFTWMRC_CTRL" = "true" ]; then
		return 0
	fi
...
base_var_init () {
	...
	if [ "none${THE_CMD_FILE_NAME}" = 'none' ]; then
		THE_CMD_FILE_NAME='leftwmrc-ctrl'
	fi

	THE_CMD_FILE_PATH="${THE_CMD_DIR_PATH}/$THE_CMD_FILE_NAME"

	## https://github.com/samwhelp/play-ubuntu-20.04-plan/blob/master/project/leftwmrc-profile/leftwmrc-ctrl/leftwmrc-ctrl
	THE_SELF_UPDATE_URL='https://raw.githubusercontent.com/samwhelp/note-about-leftwm/gh-pages/_demo/project/leftwmrc-profile/leftwmrc-ctrl/leftwmrc-ctrl'
	...
	THE_LEFTWMRC_FILE_NAME="config.toml"
